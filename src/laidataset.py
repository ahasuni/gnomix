...
def get_chm_info(genetic_map, variants_pos, chm):
    ...
    genomic_intervals = scipy.interpolate.interp1d(
...
class LAIDataset:
    ...
    def loadSyntheticFounders(self, founder_prefixes, sample_weights=None):
        ...
        if sample_weights is not None:
            ...
            self.sample_map_data["sample_weight"] = (
                self.sample_map_data["population"].map(sample_weights).astype(float)
            )
        else:
            self.sample_map_data["sample_weight"] = [1.0 / len(self.sample_map_data)] * len(
    ...
    def split_sample_map(self, ratios, split_names=None):
        ...
        for p in np.unique(self.sample_map_data["population"]):
            ...
            for s, r in enumerate(ratios):
                ...
                set_ids[s] += [
                    [pop_sample_ids.pop(idx), p, split_names[s]]
                    for idx in sorted(set_ids_idx, reverse=True)
                ]
    ...
    def create_splits(self, splits, outdir=None):
        ...
        if outdir is not None:
            for split in splits:
                ...
                self.return_split(split)[["sample", "population"]].to_csv(
